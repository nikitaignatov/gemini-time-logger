...
{
    public class FrequentWordsCommand : BaseCommand
    {
        ...
        public FrequentWordsCommand(ServiceManager svc)
        {
            this.svc = svc;
            ...
            p.Setup<bool>("temmed").SetDefault(false).Callback(x => stemmed = x).WithDescription("Performs porter stemming on the words.");
